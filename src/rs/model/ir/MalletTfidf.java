...
public class MalletTfidf extends Model {
...
//			System.out.println("docIdx: " + docIdx + "\n" + fs.toString());
	 ...
	 * @param freq
	 */
	public static void countTf(int[] tokens, int[] terms, int[] freq) {
		Arrays.sort(tokens);
		Arrays.fill(freq, 0);
		int si = 0, ti = 0;
		terms[ti] = tokens[si];
...
//		String targetFile = "dataset/vlc/task1_target.en.title.f2.txt";
		String solutionFile = "dataset/vlc/task1_solution.en.f8.tfidf.txt";
		...
		Model mt = new MalletTfidf(documents);
		Task1Solution solver = new Task1Solution(mt);
