...
public class Task1Evaluation extends EvaluationProcedure {
	...
	public Double[] runRKL(ResourceName userLabelsName, ResourceName targetLabelsName, ResourceLoader loader) 
        ...
        try {
        	...
        	double tempLastRank = 0., totalLastRank = 0.;
        	...
        	while ((targetLine = targetLabels.readLine()) != null) {
        		lineCount++;
        		if ((userLine = userLabels.readLine()) == null) throw new AlgorithmErrorException("Solution file has an incorrect format at line: " 
        				+ lineCount +" ! Solution file does not have appropriate number of rows !");		// user line end!! must throw exception
            	...
            	if (targetLecture.compareTo(userLecture) != 0) throw new AlgorithmErrorException("Solution file has an incorrect format at line: " + lineCount 
            			+"! Recommendation list for lecture: " + targetLecture + " was expecting !");		// different lecture
            	...
            	for (int i = 0 ; i < u_most ; ++i)
            		if (i < userSplit.length) {
            			user[i] = Integer.valueOf(userSplit[i]);
            			userRank.put(Integer.valueOf(userSplit[i]), i+1);
            		}
            		else
            			user[i] = -1;
            	...
            	int m = target.length;
            	tempLastRank = 0;
            	for(int i=0; i<Math.min(m, z_max); i++) 
            		if(userRank.containsKey(target[i])) 
            			tempLastRank = Math.max(tempLastRank, userRank.get(target[i]));
            		else 
            			tempLastRank = u_most;
            		
            	totalLastRank += tempLastRank;
        	}

        	return new Double[] {totalLastRank /= lectureCount};
        }catch (IOException e) {
    	...
    	throws TunedTesterException, AlgorithmErrorException
    {
            ...
            try {
            	...
            	while ((targetLine = targetLabels.readLine()) != null) {
                	...
                	if (targetLecture.compareTo(userLecture) != 0) throw new AlgorithmErrorException("Solution file has an incorrect format at line: " 
                			+ lineCount +"! Recommendation list for lecture: " + targetLecture + " was expecting !");		// different lecture
